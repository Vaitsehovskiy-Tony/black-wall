...
export const SliderBlock = ({ content, projectsList }) => {
  const slides = [
    {
      id: 1,
  ...
  if (isNarrow.matches) {
    return (
      <section className="slider__wrapper">
        <Slider slides={slides} content={content} projectsList={projectsList} />
      </section>
    );
  } else {
    return (
      <section className="carousel-main">
        <Carousel content={content} projectsList={projectsList} />{" "}
      </section>
    );
  }
};
